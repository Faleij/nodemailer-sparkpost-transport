...
SparkPostTransport.prototype.send = function send(payload, callback) {
  ...
  email.transmissionBody.recipients = ['to', 'cc', 'bcc'].reduce((arr, key) => arr.concat([].concat(payload.data[key]).reduce((p, address) => p.concat((parseEmail(address) || [])), []).map(recipient => ({
    address: recipient
  }))), [].concat(email.transmissionBody.recipients ? email.transmissionBody.recipients : []));
